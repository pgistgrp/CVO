...
<!--    <p><a href="userhome.do?workflowId=${requestScope['org.pgist.wfengine.WORKFLOW_ID']}">Back to Moderator Control Panel</a></p> -->
    ...
    <div id="overview" class="box2">
      ...
      <pg:termHighlight styleClass="glossHighlight" url="glossaryView.do?id=">Group concern keywords/phrases into categories to highlight similar ideas.  When you are done, share your categories using the "Publish" button below the categories list.</pg:termHighlight>
      <p>
        <a href="#" onclick="Effect.toggle('hiddenRM','blind'); return false"><pg:termHighlight styleClass="glossHighlight" url="glossaryView.do?id=">Read detailed instructions and see an example</pg:termHighlight></a>
      </p>
      <p id="hiddenRM" style="display:none">
        <pg:termHighlight styleClass="glossHighlight" url="glossaryView.do?id=">Scan the keywords in the right column to gain a sense of what keywords are similar to one another. In the textbox at the top of the left column, insert a category label that generalizes over those keywords. You may create a new label or use one of the existing keywords.  Click the "Add Category" button to insert the label into the left column. Click on the label makes it 'active'. In the right column, click on arrows for keywords to move them into the 'active' category in the center column.  Clicking on an arrow in the center column moves the keyword(s) back to the right column.<br><br>For the active category label, you may add a description in the textbox at the bottom of the left column.<br><br>When finished, click the "Publish" button at bottom of the left column.<br><br>Category Example:   Climate impacts might include changes in precipitation. Changes in precipitation might include more rainfall. More rainfall might include more flooding.</pg:termHighlight>
      </p>
    </div>
