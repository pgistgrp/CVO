...
tinyMCE.init({
    ...
    function setThemeTitle(themetitle){
      ...
      tree1.loadXML("/catsTree.do?bctId=${bct.id}");
      tree1.bctId = bctId;
...
/**/      
		...
		function getThemes(themeId){
			...
			CSTAgent.getThemes({bctId:bctId,asHTML:true}, {
		...
		function saveSummary(theTheme, theSummary){
			...
			//alert("bctId: " + bctId + " themeId: " + themeId + " summary:  " + summaryString + " tags: " + summaryTags);
			Util.loading(true,"Working")
			CSTAgent.saveSummary({bctId:bctId, themeId:themeId, summary:summaryString, description: "discuss concerns about", tags: summaryTags}, {
			callback:function(data){
				if (data.successful){
		...
		function getOrphanTags(){
				CSTAgent.getOrphanTags({bctId:bctId, count: 1000000000}, {
				callback:function(data){
						if (data.successful){
		...
		function getTags(categoryId, page, type, orphanpage){
			Util.loading(true,"Working")
			CSTAgent.getTags({bctId:bctId, categoryId:categoryId, page:page, count: 1000000000, orphanCount: 1000000000, type: type, orphanPage:orphanpage}, {
			callback:function(data){
				if (data.successful){
					if (type == 0){			
	...
	function addcategory(){
		...
		if(document.getElementById("newcatetext").value != ""){
			...
			CSTAgent.addCategory({bctId:bctId, parentId:parentId, name:catname},{
				callback:function(data){
					if (data.successful){ 
	...
	function moveNodeHandler(sourceO, targetO){
		params = {bctId: bctId, categoryId: sourceO.dataId};
