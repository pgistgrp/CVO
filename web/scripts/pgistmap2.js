 ...
 * Zoom to coordinats of given set of points, or if it's not given, do with
 * the Map's coords.
 * @return
...
PGISTMapEditor.prototype.scaleToCoords=function(crds, useextent){
	...
	for(var i=0; i<thecoords.length; i++){
		...
		for(var j=0; j<thecoords[i].length; j++){

			if(thecoords[i][j].x < minx) minx = thecoords[i][j].x;

			if(thecoords[i][j].x > maxx) maxx = thecoords[i][j].x;

			if(thecoords[i][j].y < miny) miny = thecoords[i][j].y;

			if(thecoords[i][j].y > maxy) maxy = thecoords[i][j].y;
