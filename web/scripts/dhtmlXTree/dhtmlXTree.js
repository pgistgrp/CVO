...
function dhtmlXTreeObject(htmlObject,width,height,rootId){
	...
	this.htmlNode=new dhtmlXTreeItemObject(0,"",0,this,0,0,this.rootId);
...
function dhtmlXTreeItemObject(itemId,itemText,parentObject,treeObject,actionHandler,mode,dataId){

	setmessage("new item: id=" + itemId + "; dataId=" + dataId + "; parentId=" + parentObject.id + "\n");
...
//prepare a new dhtmlXTreeItemObject and add to the right location 
dhtmlXTreeObject.prototype._attachChildNode=function(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,beforeNode){
	...
	var Count=parentObject.childsCount;	//points to a new availabe position 
	var Nodes=parentObject.childNodes;
	
	if(beforeNode)
	{
		var ik,jk;
		for(ik=0;ik<Count;ik++)		//the following is to prepare a location for the new node
	...
	Nodes[Count]=new dhtmlXTreeItemObject(this.getNextId(),itemText,parentObject,this,itemActionHandler,1,itemId);
	...
	var tr=this._drawNewTr(Nodes[Count].htmlNode);	//generate the presentation for this node
...
dhtmlXTreeObject.prototype._parseXMLTree=function(dhtmlObject,node,parentId,level){
	...
	if(!dhtmlObject.parsCount)dhtmlObject.parsCount=1;
	else dhtmlObject.parsCount++;
	...
	for(var i=0;i<node.childNodes.length;i++)
	{
		if((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "item"))
		{
			...
			if((!dhtmlObject.waitUpdateXML)||(dhtmlObject.waitUpdateXML.toString().search(","+cId+",")!=-1))
			{
				...
				if((!dhtmlObject.slowParse)||(dhtmlObject.waitUpdateXML))
				{
					//zcall=dhtmlObject._parseXMLTree(dhtmlObject,node.childNodes[i],cId,1);
					  zcall=dhtmlObject._parseXMLTree(dhtmlObject,node.childNodes[i],newNode.id,1);
				}
				else{
			...
			else dhtmlObject._parseXMLTree(dhtmlObject,node.childNodes[i],newNode.id,1);//cId
		}
		else
...
dhtmlXTreeObject.prototype._clearAllSelection=function(){
	for(var i=0;i<this.globalNodeStorage.length;i++)
	{
		var z = this.globalNodeStorage[i];
		if(z.htmlNode)
			z.htmlNode.childNodes[0].childNodes[0].childNodes[3].childNodes[0].className="standartTreeRow";
	}	
};

dhtmlXTreeObject.prototype.unSelectAll=function(){
	this._clearAllSelection();
	this.lastSelected=null;
};


dhtmlXTreeObject.prototype._selectSameItem=function(node){//_highlightByDataId
	this._clearAllSelection();
	
	var dataId = node.dataId; var id = node.id;
	for(var i=0;i<this.globalNodeStorage.length;i++)
		if(this.globalNodeStorage[i].dataId==dataId)
		{
			var z = this.globalNodeStorage[i];
			if(this.globalNodeStorage[i].id!=id)
				z.htmlNode.childNodes[0].childNodes[0].childNodes[3].childNodes[0].className="highlightTreeRow";
			else
				z.htmlNode.childNodes[0].childNodes[0].childNodes[3].childNodes[0].className="selectedTreeRow";
		}
	
	return;
}

dhtmlXTreeObject.prototype._selectItems=function(nodes){
	setmessage("-------_selectItems--------\n");
	if(this.lastSelected){
		this._unselectItem(this.lastSelected.parentObject);
	}
...
dhtmlXTreeObject.prototype._createItem=function(acheck,itemObject,mode){
	...
	itemObject.span.innerHTML=itemObject.label;// + "<a href=\"javascript:alert('delete')\">[delete]</a>";
...
function setmessage(msg){
	if(document.getElementById("outputdata"))
		document.getElementById("outputdata").value += msg;
}

dhtmlXTreeObject.prototype.getNextId=function(){
	this.largestId++;
	return "i_" + this.largestId;
}
...
/*	//------------------------equavelant to this._updateNodeOtherOccur(targetObject,itemObject);
	var total = this.globalNodeStorage.length;
	for(var i=0; i<total; i++){
		if(this.globalNodeStorage[i].dataId == targetObject.dataId && this.globalNodeStorage[i].id != targetObject.id)
		{
			this._cloneNode(itemObject, this.globalNodeStorage[i], itemObject.childsCount);
		}
	}
	//------------------------
*/

	this.openItem(newnode.id);

	return newnode.id;
}


//this is to attach the itemObject and all its children to the parentObject
dhtmlXTreeObject.prototype._cloneNode=function(itemObject, parentObject, total){
	//setmessage("current:" + itemObject.label + "\ntarget:" + parentObject.label + "\n\n");
	var newitem = this.insertNewItem(parentObject.id,itemObject.dataId,itemObject.label,0,0,0,0,0,0);
	newitem.dataId = itemObject.dataId;
	if(total > 0){
		for(var i=0; i<total; i++){
			this._cloneNode(itemObject.childNodes[i], newitem, itemObject.childNodes[i].childsCount);//
		}
	}
	return newitem;
}

//update nodes similar(sharing same dataId) to itemObject, only one top-level child node
dhtmlXTreeObject.prototype._updateNodeOtherOccur=function(itemObject,newChildObject){
	var total = this.globalNodeStorage.length;
	for(var i=0; i<total; i++){
		if(this.globalNodeStorage[i].dataId == itemObject.dataId && this.globalNodeStorage[i].id != itemObject.id)
		{
			this._cloneNode(newChildObject, this.globalNodeStorage[i],newChildObject.childsCount);
		}
	}
}
...
dhtmlXTreeObject.prototype._moveNodeTo=function(itemObject,targetObject,beforeNode){
	...
	else
	{
		...
		Nodes[Count]=itemObject;	//this is to append to the target location
	...
	if(!oldTree.dpcpy){
		...
		if(itemObject.parentObject)this._deleteNodeOtherOccur(itemObject.parentObject, itemObject);
	}
...
dhtmlXTreeObject.prototype._checkParenNodes=function(itemId,htmlObject,shtmlObject){	//check if htmlObject is ancestor of shtmlObject
	if(shtmlObject){
...
//first call _checkParenNodes to see direct parency, 
//then use dataId to check indirect parency
dhtmlXTreeObject.prototype._checkAnyParency=function(itemId,htmlObject,shtmlObject){
	if(!this._checkParenNodes(itemId,htmlObject,shtmlObject))
		//if(!this._checkInOffspring(shtmlObject,htmlObject,shtmlObject.childsCount))
			if(!this._checkInAncestor(shtmlObject,htmlObject))
				if(!this._checkSibling(shtmlObject,htmlObject))
					return 0;
	
	return 1;
};

//check if targetObject is an offsprint of the checkobject
dhtmlXTreeObject.prototype._checkInOffspring=function(checkObject,targetObject,checkChildCount){ 
	if(checkChildCount > 0){
		var isOffspring = false;
		for(var i=0; i<checkChildCount; i++){
			if(checkObject.childNodes[i].dataId == targetObject.dataId){
				isOffspring = true;
				break
			}else
				isOffspring = this._checkInOffspring(checkObject.childNodes[i], targetObject, checkObject.childNodes[i].childsCount);
		}
		return isOffspring;
	}
...
////check if targetObject is an ancestor of the checkobject
dhtmlXTreeObject.prototype._checkInAncestor=function(checkObject,targetObject){
	if(!targetObject)return 0;
	
	if(checkObject.dataId == targetObject.dataId || this._checkInOffspring(checkObject,targetObject,checkObject.childsCount) )
		return 1;
	else
		return this._checkInAncestor(checkObject,targetObject.parentObject);
};

//check if targetObject is an sibling of the checkobject
dhtmlXTreeObject.prototype._checkSibling=function(checkObject,targetObject){ 
	//setmessage("|csb:" + targetObject + "|");
	for(var i=0; i<targetObject.childsCount; i++){
		//setmessage(targetObject.childNodes[i].dataId + "; ");
		if(targetObject.childNodes[i].dataId == checkObject.dataId)
			return 1;
	}
	return 0;
};
...
//G: delete selected item, if any
dhtmlXTreeObject.prototype.deleteSelectedItem=function(itemId){
	
	var reftimes = 0;
	if(this.getSelectedItemId() != ""){
		if(this.lastSelected.parentObject.parentObject){//if it's not a top level
			var toBeSelected = this.lastSelected.parentObject.parentObject.id;
			//this.deleteItem(this.getSelectedItemId(), "true");
			this._deleteNodeOtherOccur(this.lastSelected.parentObject.parentObject, 
...
//update nodes similar(sharing same dataId) to itemObject, only one top-level child node
dhtmlXTreeObject.prototype._deleteNodeOtherOccur=function(parentObj,delObj){
	if(!parentObj)return 0;
	
	var total = this.globalNodeStorage.length;
	var reftimes = 0;
	for(var i=0; i<total; i++){
		if(this.globalNodeStorage[i].dataId == parentObj.dataId)
		{
			reftimes++;
			for(var j=0; j<this.globalNodeStorage[i].childsCount; j++)
				if(this.globalNodeStorage[i].childNodes[j].dataId == delObj.dataId){
					this.deleteItem( this.globalNodeStorage[i].childNodes[j].id );
					break;
				}
		}
	}
...
dhtmlXTreeObject.prototype._deleteItem=function(itemId,selectParent,skip){
	...
	if(selectParent)this.selectItem(this.getParentId(this.getSelectedItemId()),1);
		...
		if(this.lastSelected && sNode==this.lastSelected.parentObject)
			this.lastSelected=null;
	}
	if(!skip){
...
dhtmlXTreeObject.prototype.switchCopyMode=function(){
	this.dragger.copyMode = !this.dragger.copyMode;
}
...
dhtmlXTreeObject.prototype._dragIn=function(htmlObject,shtmlObject,x,y){
	if(!htmlObject.parentObject)
	{
		...
		//setmessage("| dragin: " + htmlObject);
		//if(checkSibling(shtamlObject, htmlObject.htmlNode))return 0;
		return htmlObject;
	}
	
	if((!this._checkAnyParency(shtmlObject.parentObject.id,htmlObject.parentObject,shtmlObject.parentObject))
		&&(htmlObject.parentObject.id!=shtmlObject.parentObject.id)
		&&(htmlObject.parentObject.dataId!=shtmlObject.parentObject.dataId))
	{
